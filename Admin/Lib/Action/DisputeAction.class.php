...
// 设置模块-争议管理
class DisputeAction extends AllOrderAction {
    	...
    	if(!empty($id)&&!empty($commodity_id)&&!empty($count_num))
    	{
    		...
    		if(empty($fInfo))
    		{
	    		...
	    		if ($re) {
		            ...
		            $this->assign('jumpUrl', U("Dispute/commodityList/id/".$order_id));
		            $this->success('编辑成功!');
		        } else {
		            //错误提示
    	...
    	if(!empty($id)&&!empty($commodity_id)&&!empty($count_num))
    	{
    		...
    		if(empty($fInfo))
    		{
	    		...
	    		if ($re) {
			    	...
			    	$data['amount'] = $amount;
			    	$footprints->save($data);
		            $this->assign('jumpUrl', U("Dispute/commodityList/id/".$id));
		            $this->success('添加成功!');
		        } else {
		            //错误提示
		            $this->error('添加失败!');
