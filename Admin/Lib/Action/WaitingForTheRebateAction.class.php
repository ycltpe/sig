...
// 设置模块-等待回扣管理
class WaitingForTheRebateAction extends AllOrderAction {
    	...
    	if(!empty($id)&&!empty($commodity_id)&&!empty($count_num))
    	{
    		...
    		if(empty($fInfo))
    		{
	    		...
	    		if ($re) {
		            ...
		            $this->assign('jumpUrl', U("WaitingForTheRebate/commodityList/id/".$order_id));
		            $this->success('编辑成功!');
		        } else {
		            //错误提示
    	...
    	if(!empty($id)&&!empty($commodity_id)&&!empty($count_num))
    	{
    		...
    		if(empty($fInfo))
    		{
	    		...
	    		if ($re) {
			    	...
			    	$data['amount'] = $amount;
			    	$footprints->save($data);
		            $this->assign('jumpUrl', U("WaitingForTheRebate/commodityList/id/".$id));
		            $this->success('添加成功!');
		        } else {
		            //错误提示
		            $this->error('添加失败!');
