...
// 设置模块-未确认管理
class NotRecognisedAction extends AllOrderAction {
    ...
    public function update() {
    	...
    	if(!empty($id))
    	{
	        ...
	        if (false !== $list) {
	            ...
	            $this->assign('jumpUrl', U("NotRecognised/"));
	            $this->success('编辑成功!');
	        } else {
	            //错误提示
	            $this->error('编辑失败!');
    	...
    	if(!empty($id)&&!empty($commodity_id)&&!empty($count_num))
    	{
    		...
    		if(empty($fInfo))
    		{
	    		...
	    		if ($re) {
		            ...
		            $this->assign('jumpUrl', U("NotRecognised/commodityList/id/".$order_id));
		            $this->success('编辑成功!');
		        } else {
		            //错误提示
    	...
    	if(!empty($id)&&!empty($commodity_id)&&!empty($count_num))
    	{
    		...
    		if(empty($fInfo))
    		{
	    		...
	    		if ($re) {
			    	...
			    	$data['amount'] = $amount;
			    	$footprints->save($data);
		            $this->assign('jumpUrl', U("NotRecognised/commodityList/id/".$id));
		            $this->success('添加成功!');
		        } else {
		            //错误提示
		            $this->error('添加失败!');
