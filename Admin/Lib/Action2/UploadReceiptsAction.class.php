...
// 设置模块-上传收据管理
class UploadReceiptsAction extends AllOrderAction {
    ...
    public function update() {
    	...
    	if(!empty($id))
    	{
	        ...
	        if (false !== $list) {
	            $this->assign('jumpUrl', U("UploadReceipts/"));
	            $this->success('编辑成功!');
	        } else {
	            //错误提示
	            $this->error('编辑失败!');
    	...
    	if(!empty($id)&&!empty($commodity_id)&&!empty($count_num))
    	{
    		...
    		if(empty($fInfo))
    		{
	    		...
	    		if ($re) {
		            ...
		            $this->assign('jumpUrl', U("UploadReceipts/commodityList/id/".$order_id));
		            $this->success('编辑成功!');
		        } else {
		            //错误提示
    	...
    	if(!empty($id)&&!empty($commodity_id)&&!empty($count_num))
    	{
    		...
    		if(empty($fInfo))
    		{
	    		...
	    		if ($re) {
			    	...
			    	$data['amount'] = $amount;
			    	$footprints->save($data);
		            $this->assign('jumpUrl', U("UploadReceipts/commodityList/id/".$id));
		            $this->success('添加成功!');
		        } else {
		            //错误提示
		            $this->error('添加失败!');
