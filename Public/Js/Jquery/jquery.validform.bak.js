...
(function($){
				...
				if(!flag){
					if($(this).attr("ignore")==="ignore" && ( $(this).val()==="" || $(this).val()===$(this).attr("tip") )){
						if(settings.tiptype==2){
							//$(this).parent().next().find(".checktip").removeClass().addClass("checktip").text("");
							$(this).next(".checktip").removeClass().addClass("checktip").text("");
					...
					if($(this).attr("ajaxurl")){
						...
						$.ajax({
							...
							success: function(s){
								if(s=="y"){
									...
									errorobj=null;
									$.fn.Validform.sn.showmsg(inputobj.attr("rightmsg")||tipmsg.r,settings.tiptype,{obj:inputobj,type:2,sweep:settings.tipSweep},"hide");
								}else{
						...
						errorobj=null;
						$.fn.Validform.sn.showmsg($(this).attr("rightmsg")||tipmsg.r,settings.tiptype,{obj:$(this),type:2,sweep:settings.tipSweep},"hide");
					}
				}
				...
				$this.find("[datatype]").each(function(){
					...
					if($(this).attr("ajaxurl")){
						if($(this).attr("valid")!="true"){
							...
							$.fn.Validform.sn.showmsg(thisobj.attr("rightmsg")||tipmsg.r,settings.tiptype,{obj:$(this),type:2,sweep:settings.tipSweep},"hide");
							flag=true;
						}
					}
				});
	...
	$.fn.Validform.sn={
		...
		showmsg:function(msg,type,o,show){//o:{obj:当前对象, type:1=>正在检测 | 2=>通过}, show用来判断tiptype=1的情况下是否弹出信息框;
			...
			if(type==2 && o.obj){
				//o.obj.parent().next().find(".checktip").text(msg);
				//this.cssctl(o.obj.parent().next().find(".checktip"),o.type);
				o.obj.next(".checktip").text(msg);
				this.cssctl(o.obj.next(".checktip"),o.type);
			}
		...
		checkform:function(obj,parentobj,tiptype,show){//show用来判断是表达提交还是blur事件引发的检测;
			...
			if(obj.is("[datatype='radio']")){  //判断radio表单元素;
				...
				errorobj=null;
				this.showmsg(obj.attr("rightmsg")||tipmsg.r,tiptype.type,{obj:obj,type:2,sweep:tiptype.sweep},"hide");
				return "radio";
			}
		...
		cssctl:function(obj,status){
			switch(status){
				case 1:
					obj.removeClass("check_right check_error").addClass("checktip check_loading");//checking;
					break;
				case 2:
					obj.removeClass("check_error check_loading").addClass("checktip check_right");//passed;
					break;
				case 4:
					obj.removeClass("check_right check_error check_loading").addClass("checktip");//for ignore;
					break;
				default:
					obj.removeClass("check_right check_loading").addClass("checktip check_error");//wrong;
			}	
		}
	...
	$.Datatype={
		"match":/^(.+)(\d+)-(\d+)$/,
		"*":/.+/,	 /* 确认密码 */
		"*6-16":/^.{6,16}$/,	 /* 输入密码6-16位之间*/
		"n":/^\d+$/,	/* 数字输入*/
		"s":/^[\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]+$/m,	/* 数字和字母 可同时输入*/
		"s6-18":/^[\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]{6,18}$/,	/* 字符长度6-18位之间 */
		"p":/^[0-9]{6}$/,	 /* 输入6位数字 */
		"m":/^13[0-9]{9}$|15[0-9]{9}$|18[0-9]{9}$/,	/* 手机验证 */
		"e":/^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/,
		"yhk":/^[\d]{16,19}$/,
		"i":/^(\d{15}|\d{18}|\d{17}[a-zA-Z]{1})$/,
		"mz":/^[\u4e00-\u9fa5]{2,10}$/
	};
})(jQuery);